...
//   );
// };
  var reelData = useSelector(selectAllReels);
  console.log('reelData', reelData)
  if (route.params?.reelId) {
    reelData = [reelData.find(reel => reel.reels_id === route.params?.reelId), ...reelData.filter(reel => reel.reels_id !== route.params?.reelId)];
  }
  ...
  const likeReel = id => {
    let selectedReel = reelData.find(item => item.reels_id == id);
    console.log('selectedReel',selectedReel)
    let isLiked = selectedReel?.isLiked === 1 ? 0 : 1;
    let updateReelData = { 
      reel_id:id,
      reaction_id:1
    };
    let likecount = selectedReel.like_count || 0;
    likecount = selectedReel.isLiked === 1 ? likecount - 1:likecount +1;

    dispatch(reelLikeAction(updateReelData),).then(()=>{
      dispatch(updateReel({ id, changes:{isLiked:isLiked,like_count: likecount }}))
    })
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#370617', }}>
      <StatusBar barStyle="light-content" />
      <SwiperFlatList
        vertical={true}
        data={reelData}
        renderItem={({ item, index }) => <ReelBox item={item} index={index} currentIndex={currentIndex} likeReel={likeReel} />}
